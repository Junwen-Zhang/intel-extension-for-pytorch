...
IPEX_DEFINE_DISPATCH(tpp_ffn_swiglu_kernel_stub);
...
at::Tensor tpp_ffn_swiglu_forward_cpu(
    const at::Tensor& t_in,
    const at::Tensor& t_wt_gate,
    const at::Tensor& t_bias_gate,
    const at::Tensor& t_wt_up,
  ...
  return tpp_ffn_swiglu_kernel_stub(
      kCPU,
      t_in,
      t_wt_gate,
      t_bias_gate,
...
TORCH_LIBRARY_FRAGMENT(torch_ipex, m) {
  m.def(
      "tpp_ffn_swiglu(Tensor t_in, Tensor t_wt_gate, Tensor t_bias_gate, Tensor t_wt_up, Tensor t_bias_up, Tensor t_wt_down, Tensor t_bias_down)-> Tensor out");
  m.impl(
      "tpp_ffn_swiglu",
      c10::DispatchKey::CPU,
      torch_ipex::cpu::tpp_ffn_swiglu_forward_cpu);
}
